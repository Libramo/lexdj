...
async function getFilterOptions() {
  const [docTypes, ministries] = await Promise.all([
    ...
    db
      .select({ value: laws.ministry, count: count() })
      .from(laws)
...
export default async function RecherchePage({ searchParams }: Props) {
  ...
  if (q)
    conditions.push(
      `to_tsvector('french', COALESCE(title,'') || ' ' || COALESCE(intro_text,'') || ' ' || COALESCE(full_text,'')) @@ plainto_tsquery('french', '${q.replace(/'/g, "''")}')`,
    );
  if (typeFilter)
  ...
  const orderClause =
    sort === "date_desc"
      ...
      : sort === "date_asc"
        ...
        : q
          ? `ORDER BY ts_rank(to_tsvector('french', COALESCE(title,'') || ' ' || COALESCE(intro_text,'') || ' ' || COALESCE(full_text,'')), plainto_tsquery('french', '${q.replace(/'/g, "''")}')) DESC`
          : "ORDER BY publication_date DESC NULLS LAST";
  ...
  const [results, totalResult] = await Promise.all([
    ...
    q || hasFilters
      ? db.execute(sql.raw(`SELECT COUNT(*) as total FROM laws ${whereClause}`))
                ...
                <div className="flex items-center gap-1.5 text-xs text-[#888]">
                  ...
                  {[
                    ...
                    <Link
                      key={s.value}
                      href={pageUrl(1)
                        .replace(`sort=${sort}`, `sort=${s.value}`)
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-semibold text-[#111] leading-snug group-hover:text-[#1A3A5C] transition-colors mb-1.5">
                      <HighlightedText text={law.title ?? "Sans titre"} q={q} />
                    </p>

                    {/* Excerpt */}
                    {law.intro_text && (
                      <p className="text-xs text-[#888] leading-relaxed line-clamp-2 font-light mb-2">
                        <HighlightedText
                          text={law.intro_text.slice(0, 220)}
                          q={q}
                        />
                      </p>
                    )}
